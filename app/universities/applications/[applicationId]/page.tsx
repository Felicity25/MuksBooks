...
export default function ApplicationDetailPage() {
  ...
  const addPersonalDeadlineTask = () => {
    if (!application?.userDeadline) return
    addTask('Submit university application by personal deadline', application.userDeadline)
  }
  ...
  const isCanonical = Boolean(programme && institution)
  ...
  return <main className="mx-auto w-full max-w-6xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <header className="flex flex-wrap items-start justify-between gap-4"><div><div className="flex flex-wrap items-center gap-2"><p className="text-sm font-semibold uppercase text-sky-700">{institutionName}</p><span className="rounded bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-600">{application.dataOrigin === 'USER_ENTERED' ? 'Learner entered' : 'Official catalogue'}</span></div><h1 className="mt-1 text-3xl font-semibold text-slate-950">{programmeName}</h1><p className="mt-2 text-slate-600">{institution?.city ? `${institution.city}, ` : ''}{country} • {application.intakeYear} intake</p></div>{isCanonical ? <Link href={`/universities/${institution!.id}/${programme!.id}`} className="inline-flex items-center gap-2 text-sm font-medium text-sky-700">Programme details <ExternalLink className="h-4 w-4" /></Link> : null}</header>
    ...
    <Card className="p-5"><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700">Personal deadline<input type="date" value={application.userDeadline ?? ''} onChange={(event) => update({ userDeadline: event.target.value || undefined }, event.target.value ? `Personal deadline set to ${event.target.value}.` : 'Personal deadline removed.')} className="mt-1 block h-10 w-full rounded-md border border-slate-300 bg-white px-3" /></label><label className="text-sm font-medium text-slate-700">Deadline note<input value={application.userDeadlineNote ?? ''} onChange={(event) => update({ userDeadlineNote: event.target.value || undefined })} placeholder="Why this date applies" className="mt-1 block h-10 w-full rounded-md border border-slate-300 bg-white px-3" /></label></div><div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-slate-500">Your planning date is stored separately and never replaces an official published deadline.</p>{application.userDeadline ? <Button type="button" variant="secondary" size="sm" disabled={application.tasks.some((task) => task.title === 'Submit university application by personal deadline' && task.dueAt === application.userDeadline)} onClick={addPersonalDeadlineTask}><CalendarPlus className="mr-2 h-4 w-4" />Create task</Button> : null}</div></Card>

    <div className="grid gap-6 lg:grid-cols-[1.15fr_.85fr]">
      <section className="space-y-6">
        {isCanonical ? <AdmissionsReadinessPanel
        ...
        /> : <Card className="p-5"><h2 className="text-lg font-semibold text-slate-950">Admissions readiness not indexed</h2><p className="mt-2 text-sm text-slate-600">This learner-entered application is not linked to a verified catalogue programme. Use the official application URL and record requirements as tasks or notes.</p></Card>}
