...
import { ArrowRight, Filter, Globe2, Search, Sparkles, X } from 'lucide-react'
...
import { DEFAULT_UNIVERSITY_SEARCH_STATE, hasActiveUniversitySearch, parseUniversitySearchState, serializeUniversitySearchState, updateUniversitySearchState, type UniversitySearchState } from '@/lib/universities/search-state'
...
export default function UniversitiesPage() {
  ...
  const [queryState, setQueryState] = useState<UniversitySearchState>(DEFAULT_UNIVERSITY_SEARCH_STATE)
  ...
  useEffect(() => {
    const restoreFromUrl = () => setQueryState(parseUniversitySearchState(new URLSearchParams(window.location.search)))
    restoreFromUrl()
    window.addEventListener('popstate', restoreFromUrl)
    return () => window.removeEventListener('popstate', restoreFromUrl)
  }, [])

  const setSearchState = (updates: Partial<UniversitySearchState>) => {
    setQueryState((current) => {
      const next = updateUniversitySearchState(current, updates)
      const params = serializeUniversitySearchState(next).toString()
      window.history.replaceState(window.history.state, '', `${window.location.pathname}${params ? `?${params}` : ''}`)
      return next
    })
  }

  const filteredResults = useMemo(() => {
    const results = searchUniversityCatalogue(queryState.search, queryState)
    ...
    <SectionShell title="Universities" description="Find courses, universities and opportunities that fit you." contentClassName="space-y-6">
      <Card className="p-5">
        ...
        <div className="mt-2 flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2.5 focus-within:ring-2 focus-within:ring-sky-600">
          ...
          <input id="university-search" value={queryState.search} onChange={(event) => setSearchState({ search: event.target.value })} placeholder="Try Computer Science, UCT, medicine or finance" className="w-full bg-transparent text-slate-950 placeholder:text-slate-400 focus:outline-none" />
          {queryState.search ? <button type="button" onClick={() => setSearchState({ search: '' })} aria-label="Clear search" className="rounded p-1 text-slate-500 hover:bg-slate-100"><X className="h-4 w-4" /></button> : null}
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <label className="text-sm font-medium text-slate-700">Country<select value={queryState.country} onChange={(event) => setSearchState({ country: event.target.value })} className="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3">{COUNTRY_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></label>
          <label className="text-sm font-medium text-slate-700">State or region<select value={queryState.region} onChange={(event) => setSearchState({ region: event.target.value })} className="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3">{REGION_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></label>
          <label className="text-sm font-medium text-slate-700">Study area<select value={queryState.studyArea} onChange={(event) => setSearchState({ studyArea: event.target.value })} className="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3">{STUDY_AREA_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></label>
          <label className="text-sm font-medium text-slate-700">Qualification<select value={queryState.qualification} onChange={(event) => setSearchState({ qualification: event.target.value })} className="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3">{QUALIFICATION_OPTIONS.map((option) => <option key={option}>{option}</option>)}</select></label>
        </div>
        {hasActiveUniversitySearch(queryState) ? <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-600">
          {queryState.country !== 'All' ? <Button type="button" size="sm" variant="ghost" onClick={() => setSearchState({ country: 'All' })}>Clear country</Button> : null}
          {queryState.region !== 'All' ? <Button type="button" size="sm" variant="ghost" onClick={() => setSearchState({ region: 'All' })}>Clear state/region</Button> : null}
          {queryState.studyArea !== 'All' ? <Button type="button" size="sm" variant="ghost" onClick={() => setSearchState({ studyArea: 'All' })}>Clear study area</Button> : null}
          {queryState.qualification !== 'All' ? <Button type="button" size="sm" variant="ghost" onClick={() => setSearchState({ qualification: 'All' })}>Clear qualification</Button> : null}
          <Button type="button" size="sm" variant="outline" onClick={() => setSearchState(DEFAULT_UNIVERSITY_SEARCH_STATE)}>Clear all</Button>
        </div> : null}
      </Card>
      ...
      <section aria-labelledby="results-heading"><div className="flex items-center justify-between gap-3"><div><h2 id="results-heading" className="text-xl font-semibold text-slate-950">Programmes</h2><p className="mt-1 text-sm text-slate-600">{filteredResults.length} matching programme{filteredResults.length === 1 ? '' : 's'}</p></div><Filter className="h-5 w-5 text-slate-500" /></div><div className="mt-4 grid gap-4 xl:grid-cols-2">{filteredResults.slice(0, 30).map((result) => {
        ...
        return <Card key={result.programme.id} className="space-y-4 p-5"><div><Link href={`/universities/${result.institution.id}/${result.programme.id}`} className="text-lg font-semibold text-slate-950 hover:text-sky-700">{result.programme.name}</Link><Link href={`/universities/${result.institution.id}`} className="mt-1 block text-sm text-slate-600 hover:text-sky-700">{result.institution.name} • {result.institution.city}, {result.country}</Link>{result.matchReason ? <p className="mt-2 text-xs font-medium text-sky-700">Why this matched: {result.matchReason}</p> : null}</div><div className="flex flex-wrap gap-2">{result.programme.studyAreas.map((area) => <span key={area} className="rounded-full bg-slate-100 px-2 py-1 text-xs text-slate-700">{area}</span>)}</div><div className="grid gap-2 text-sm text-slate-600 sm:grid-cols-2"><p><strong className="text-slate-800">Qualification:</strong> {result.programme.degreeType}</p><p><strong className="text-slate-800">Duration:</strong> {result.programme.duration ?? 'Check official course'}</p></div>{match ? <div className="rounded-md border border-slate-200 bg-slate-50 p-3 text-sm"><p className="font-semibold text-slate-900">Your match: {match.title}</p><p className="mt-1 text-slate-600">{match.reasons[0] ?? match.checks[0]}</p></div> : <p className="text-sm text-slate-600">Add your curriculum and grades to compare your profile with published requirements.</p>}<ProgrammeActions programmeId={result.programme.id} institutionId={result.institution.id} /></Card>
      })}{!filteredResults.length ? <Card className="p-6 text-sm text-slate-600 xl:col-span-2"><p className="font-semibold text-slate-900">No universities match your search and filters.</p><p className="mt-1">Clear only the criteria you want to broaden; your other choices will stay active.</p></Card> : null}</div></section>
    </SectionShell>
  )
}