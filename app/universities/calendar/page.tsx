...
import type { DeadlineType, UniversityApplication } from '@/lib/universities/types'

type CalendarCategory = 'ALL' | 'APPLICATIONS' | 'DOCUMENTS' | 'TESTS' | 'RESULTS' | 'OFFERS'
...
export default function UniversityCalendarPage() {
  ...
  const events = [...deadlineEvents, ...resultEvents]
  ...
  return <main className="mx-auto w-full max-w-5xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
    ...
    <section className="grid gap-3 border-y border-slate-200 py-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Calendar filters">
      <label className="text-sm font-medium text-slate-700">Event type<select value={category} onChange={(event) => setCategory(event.target.value as CalendarCategory)} className="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3">{(['ALL', 'APPLICATIONS', 'DOCUMENTS', 'TESTS', 'RESULTS', 'OFFERS'] as CalendarCategory[]).map((value) => <option key={value} value={value}>{label(value)}</option>)}</select></label>
      ...
      <label className="text-sm font-medium text-slate-700">Tracked application<select value={applicationId} onChange={(event) => setApplicationId(event.target.value)} className="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3"><option value="ALL">All applications</option>{applications.map((application) => <option key={application.id} value={application.id}>{getProgramme(application.programmeId)?.name || application.customProgrammeName || 'Untitled programme'} · {getInstitution(application.institutionId)?.name || application.customInstitutionName || 'Unlisted university'}</option>)}</select></label>
    </section>
