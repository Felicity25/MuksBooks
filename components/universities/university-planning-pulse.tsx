...
import { evaluateAdmissionsReadiness } from '@/lib/universities/admissions-readiness'
...
export function UniversityPlanningPulse() {
  ...
  useEffect(() => setLearnerProfile(getLearnerProfile()), [])
  ...
  const actions = useMemo(() => applications.flatMap((application) => {
    ...
    const readiness = evaluateAdmissionsReadiness({ profile: learnerProfile, institutionId: application.institutionId, programmeId: application.programmeId, intakeYear: application.intakeYear, applicantRoute: application.applicantRoute, applicantType: application.applicantContext?.likelyApplicantType, application })
    const readinessPriority = ['TEST_REQUIRED', 'ACTION_REQUIRED', 'ACTION_NEEDED', 'TEST_RECOMMENDED', 'POSSIBLE_EXEMPTION', 'MISSING_INFORMATION']
    const readinessAction = readinessPriority.flatMap((state) => readiness.checks.filter((check) => check.state === state))[0]
    ...
    return [{ application, programme, institution, readinessAction, nextTask, nextDeadline }]
  }).sort((left, right) => String(left.nextTask?.dueAt || left.nextDeadline?.dueAt || 'z').localeCompare(String(right.nextTask?.dueAt || right.nextDeadline?.dueAt || 'z'))).slice(0, 3), [applications, learnerProfile])
